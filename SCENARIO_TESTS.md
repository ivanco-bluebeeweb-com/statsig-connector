# PST Testing Scenarios for Statsig Connector

## Part A: Authentication & Connectivity Verification
- **Scenario A1: Valid Credential Connect**: `connect_statsig_connector` saves token and validates endpoint.
...
- **Scenario B1: List Gates**: `list_gates` returns typed list with bounds.
- **Scenario B2: Get Gate**: `get_gate` returns entity details or 404 error.
- **Scenario B3: Health Audit**: `audit_gate_health` aggregates active records.

## Part C: Safe Write & Idempotency Testing
- **Scenario C1: Connection Lifecycle**: Clean disconnect via `disconnect_statsig_connector` without lingering secrets.
