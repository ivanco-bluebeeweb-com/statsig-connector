"""Connection lifecycle for Statsig Connector."""
...
@chat.function(
    "connect_statsig",
    "Connect Statsig account via credentials.",
    action_type="write",
    chain_callable=True,
    event="statsig-connector.connect_statsig",
    effects=["create:connection"],
    data_model=ConnectParams
)
async def connect_statsig(params: ConnectParams, ctx) -> ActionResult[ConnectionRecord]:
    """Connect Statsig Connector."""
    client = StatsigClient(api_key=params.api_key, base_url=params.base_url)
    await client.verify_auth()
    conns = await _load_connections(ctx)
    cid = f"conn_{uuid.uuid4().hex[:8]}"
    record = {
        "id": cid,
        "label": params.label or "Statsig Account",
...
@chat.function(
    "list_connections",
    "List connected Statsig accounts.",
    action_type="read",
    chain_callable=True,
    data_model=NoParams
)
...
@chat.function(
    "disconnect_statsig",
    "Disconnect Statsig account.",
    action_type="write",
    chain_callable=True,
    event="statsig-connector.disconnect_statsig",
    effects=["delete:connection"],
    data_model=ConnectionIdParams
)
async def disconnect_statsig(params: ConnectionIdParams, ctx) -> ActionResult[DeleteResult]:
    conns = await _load_connections(ctx)
