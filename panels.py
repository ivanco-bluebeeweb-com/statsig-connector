"""Panel UI for Statsig Connector following UI_INTERFACE_STANDARD.md."""
...
def _settings_button() -> ui.UINode:
    return ui.Button(
        ...
        size="sm",
        icon="settings",
        on_click=ui.Call("__panel__statsig_settings")
    )

def _help_modal() -> ui.UINode:
    return ui.Modal(
        trigger=ui.Button("How do I set this up?", variant="ghost", size="sm"),
        title="Connecting Statsig",
        children=[
            ui.Text(
                "1. Sign in to your Statsig dashboard and navigate to API/Integration settings.\n"
...
@ext.panel("statsig_sidebar", slot="left")
async def main_panel(ctx) -> ui.UINode:
    form = ui.Form(
        submit_label="Connect Statsig",
        action=ui.Call("connect_statsig"),
        children=[
            ui.Stack(
                direction="v",
                gap=2,
    ...
    return ui.Stack(
        ...
        gap=2,
        children=[
            ui.Heading("Statsig Connector", level=3),
            ui.Text("Connect and manage your Statsig workspace.", variant="caption"),
            ui.Divider(),
            form,
            ui.Divider(),
            _help_modal(),
